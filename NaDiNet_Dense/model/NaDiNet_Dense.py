...
class NaDiNet(nn.Module):
    def __init__(self):
        super(NaDiNet, self).__init__()

        model = getattr(models, 'densenet201')(True)
        self.block1 = nn.Sequential(*list(model.features.children())[:4])
        self.block2 = model.features.denseblock1
        self.block3 = model.features.denseblock2
        self.block4 = model.features.denseblock3
        self.block5 = model.features.denseblock4

        self.transition1 = model.features.transition1
        self.transition2 = model.features.transition2
        self.transition3 = model.features.transition3
        ...
        self.reduce3 = nn.Sequential(nn.Conv2d(256, 128, kernel_size=3, padding=1), nn.BatchNorm2d(128),
                                     nn.ReLU(inplace=True))
        self.reduce4 = nn.Sequential(nn.Conv2d(896, 128, kernel_size=3, padding=1), nn.BatchNorm2d(128),
                                     nn.ReLU(inplace=True))
        self.reduce5 = nn.Sequential(nn.Conv2d(1920, 128, kernel_size=3, padding=1), nn.BatchNorm2d(128),
                                     nn.ReLU(inplace=True))
        ...
        self.decoder5 = DIB(128, 128)
        self.decoder4 = DIB(128, 128)
        self.decoder3 = DIB(128, 128)
        self.decoder2 = DIB(128, 128)
        self.decoder1 = DIB(128, 128)
    ...
    def forward(self, x):
        ...
        # -------------Encoder-------------
        h1 = self.block1(hx)  # 96x96x64
        h2 = self.transition1(self.block2(h1))  # 48x48x128
        h3 = self.transition2(self.block3(h2))  # 24x24x256
        h4 = self.transition3(self.block4(h3))  # 12x12x896
        h5 = self.block5(h4)  # 12x12x1920
        ...
        db = self.up32(db)  # 11 -> 352

        d5 = self.outconv5(hd5)
        d5 = self.up32(d5)  # 11 -> 352

        d4 = self.outconv4(hd4)
        d4 = self.up32(d4)  # 11 -> 352

        d3 = self.outconv3(hd3)
        d3 = self.up16(d3)  # 22 -> 352

        d2 = self.outconv2(hd2)
        d2 = self.up8(d2)  # 44 -> 352
